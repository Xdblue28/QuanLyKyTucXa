    ...
    public partial class Form1 : Form
    {
        ...
        private void btnXacNhan_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection conn = DBConnect.GetConnection())
                {
                    ...
                    string checkSql = @"SELECT COUNT (*) FROM TienPhong WHERE MaSinhVien = @MaSinhVien AND MONTH(Ngay) = MONTH(@Ngay)  AND YEAR(Ngay) = YEAR(@Ngay)";

                    SqlCommand cm = new SqlCommand(checkSql, conn);
                    cm.Parameters.AddWithValue("@MaSinhVien", txtMaSinhVien.Text.Trim());
                    ...
                    string sql = @"INSERT INTO TienPhong (MaSinhVien, HoTen, SDT, Gmail, DiaChi, CCCD, IDPhong, Ngay, TienThanhToan)VALUES  (@MaSinhVien, @HoTen, @SDT, @Gmail, @DiaChi, @CCCD, @IDPhong, @Ngay, @TienThanhToan)";

                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        cmd.Parameters.AddWithValue("@MaSinhVien", txtMaSinhVien.Text);
        ...
        private void btnTimKiem_Click_2(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT * FROM TienPhong WHERE MaSinhVien = @MaSinhVien";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtMSSV.Text = dr["MaSinhVien"].ToString();
                        ...
                        txtIDPhong.Text = dr["IDPhong"].ToString();
                    }
                    else
                    {
